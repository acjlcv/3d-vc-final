...
parser.add_argument('--tag', type=str, default="Ours")

args = parser.parse_args()


# Logging
save_dir = os.path.join(args.save_dir, f'GEN_{args.tag}_%s_%d' % ('_'.join(args.categories), int(time.time())) )
...
train_dset, val_dset, test_dset = get_train_val_test_datasets(dset, train_ratio=0.70, val_ratio=0.15)
...
elif ckpt['args'].model == 'flow_matching':
    model = FlowMatch(ckpt['args']).to(args.device)
