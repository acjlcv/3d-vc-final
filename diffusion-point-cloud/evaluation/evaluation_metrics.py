...
def emd_approx(sample, ref, epsilon=0.01, max_iter=10):
    ...
    # x_exp = sample.unsqueeze(2)  # (B, N, 1, D)
    # y_exp = ref.unsqueeze(1)  # (B, 1, M, D)
    # C = torch.norm(x_exp - y_exp, dim=3, p=2)  # (B, N, M)

    # mu = torch.full((B, N), 1.0 / N, device=sample.device, dtype=sample.dtype)  # (B, N)
    # # Uniform weights on source and target points
    # nu = torch.full((B, M), 1.0 / M, device=sample.device, dtype=sample.dtype)  # (B, M)

    # # Initialize log u and log v (dual variables)
    # u = torch.zeros_like(mu)  # (B, N)
    # v = torch.zeros_like(nu)  # (B, M)

    # # K = exp(-C / epsilon)
    # K = torch.exp(-C / epsilon)  # (B, N, M)

    # for _ in range(max_iter):
    #     u = mu / (K @ v.unsqueeze(2)).squeeze(2)  # (B, N)
    #     v = nu / (K.transpose(1, 2) @ u.unsqueeze(2)).squeeze(2)  # (B, M)

    # # Transport plan gamma = diag(u) * K * diag(v)
    # U = u.unsqueeze(2)  # (B, N, 1)
    # V = v.unsqueeze(1)  # (B, 1, M)
    # gamma = U * K * V  # (B, N, M)

    # # Sinkhorn distance = sum of elementwise product of gamma and cost matrix C
    # emd = torch.sum(gamma * C, dim=(1, 2))  # (B,)
