...
class PointwiseNet(Module):
    ...
    def forward(self, x, beta: torch.Tensor, context, infer=False, debug=False):
        """
        Args:
        ...
        if debug:
            print(x.shape, beta.shape, context.shape)

        if infer:
            beta = torch.full((batch_size, ), beta.item()).float().to(x.device)
